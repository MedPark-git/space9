...
def db_config_present():
    return all(os.getenv(k) for k in REQUIRED_DB_ENV)

def database_uri():
    if not db_config_present():
        return "postgresql+psycopg://not-ready:not-ready@127.0.0.1:1/not-ready"
    from urllib.parse import quote_plus
    return (f"postgresql+psycopg://{quote_plus(os.environ['DB_USER'])}:"
...
class AuditLog(db.Model):
    ...
    login_id = db.Column(db.String(80), index=True)
...
def commit_or_rollback():
    try: db.session.commit(); return True
        ...
        db.session.rollback(); app.logger.exception("Database transaction failed"); return False

def ensure_schema_and_bootstrap():
    if not db_config_present(): raise RuntimeError("PostgreSQL 환경변수가 준비되지 않았습니다.")
    with db.engine.begin() as conn:
        conn.execute(text("SELECT pg_advisory_xact_lock(874221934)"))
        db.metadata.create_all(bind=conn)
        current = conn.execute(text("SELECT COALESCE(MAX(version), 0) FROM schema_versions")).scalar_one()
        if current < SCHEMA_VERSION:
            conn.execute(text("INSERT INTO schema_versions(version, applied_at) VALUES (:v, NOW()) ON CONFLICT DO NOTHING"), {"v": SCHEMA_VERSION})
    if User.query.count() == 0:
        ...
        db.session.add(User(login_id=admin_id.strip().lower(), name=admin_name, role="admin", password_hash=generate_password_hash(admin_pw), is_active_flag=True, must_change_password=True))
        db.session.commit()
...
def prepare_request():
    ...
    if current_user.is_authenticated and current_user.must_change_password and request.endpoint not in {"force_password_change", "logout", "not_ready"}:
...
def recent_failed_attempts(login_id):
    since = utcnow() - timedelta(minutes=10)
    return AuditLog.query.filter(AuditLog.action == "login_failed", AuditLog.login_id == login_id, AuditLog.created_at >= since).count()
...
def login():
    ...
    if request.method == "POST":
        ...
        if recent_failed_attempts(login_id) >= 5:
            flash("로그인 실패 횟수가 초과되었습니다. 10분 후 다시 시도해 주세요.", "error"); return render_template("login.html"), 429
        ...
        login_user(user); user.last_login_at = utcnow(); audit("login_success", user=user); commit_or_rollback()
        return redirect(url_for("force_password_change" if user.must_change_password else "dashboard"))
...
def users():
    if request.method == "POST":
        ...
        if not login_id or User.query.filter(db.func.lower(User.login_id) == login_id).first(): flash("로그인 ID가 비어 있거나 이미 존재합니다.", "error")
            ...
            temp_password = uuid.uuid4().hex[:8] + "!9Aa"
            user = User(login_id=login_id, name=(request.form.get("name") or login_id).strip(), department=(request.form.get("department") or "").strip(), position=(request.form.get("position") or "").strip(), role=request.form.get("role") if request.form.get("role") in {"admin", "editor", "viewer"} else "viewer", password_hash=generate_password_hash(temp_password), must_change_password=True)
            db.session.add(user); audit("user_created", user=current_user, target_type="user", target_id=user.id, detail=f"login_id={login_id}")
...
def toggle_user(user_id):
    ...
    if user.id == current_user.id: flash("현재 로그인한 관리자 계정은 비활성화할 수 없습니다.", "error")
        ...
        user.is_active_flag = not user.is_active_flag; audit("user_status_changed", user=current_user, target_type="user", target_id=user.id, detail=f"active={user.is_active_flag}"); commit_or_rollback(); flash("사용자 상태가 변경되었습니다.", "success")
...
def change_user_role(user_id):
    ...
    if user.id == current_user.id and role != "admin": flash("현재 로그인한 관리자 본인의 관리자 권한은 해제할 수 없습니다.", "error")
        ...
        old_role = user.role; user.role = role; audit("user_role_changed", user=current_user, target_type="user", target_id=user.id, detail=f"{old_role}->{role}"); commit_or_rollback(); flash("사용자 권한이 변경되었습니다.", "success")
    return redirect(url_for("users"))
...
def health():
    ...
    try:
        db.session.execute(text("SELECT 1")); result["database"] = True
        db.session.execute(text("CREATE TEMP TABLE IF NOT EXISTS health_write_check (id integer) ON COMMIT DROP")); db.session.execute(text("INSERT INTO health_write_check(id) VALUES (1)")); db.session.rollback()
        result.update(status="ok", database_writable=True, application_ready=True); return jsonify(result), 200
    except Exception:
        db.session.rollback(); return jsonify(result), 503
